...
void myServer::onMyConnect(){
    ...
    connect(tcpSocket[0],&QTcpSocket::readyRead,[=](){
        ...
        else if("getinfo_group"== recvinfo.function_name)
            getinfo_group(tcpSocket,recvinfo);
        else
            qDebug() << "zen me shier?";
    });
}
...
void myServer::registeruser(QTcpSocket* tcpSocket[],Protocol recvinfo){
    ...
    if(!sqlquery.next())
    {
        ...
        sqlquery.clear();
        Func func;
        //const QString &username, const QString &password, const QByteArray &image, int question, const QString &answer
        int userid = func.account_register(recvinfo.username,recvinfo.userpwd,recvinfo.ba, recvinfo.friendid,recvinfo.msg);
...
void myServer::wantSendFile(QTcpSocket* tcpSocket[],Protocol recvinfo){
    ...
    db.open();
    Func func;
    func.insertNewMessage(recvinfo.userid,recvinfo.friendid,QString("去码头搞点文件！"),recvinfo.tim);
}
...
void myServer::getinfo(QTcpSocket *tcpSocket[], Protocol recvinfo){
    Func func;
        ...
        Protocol sendinfo;
        QByteArray pic = func.getPhoto(recvinfo.userid);
        if(!friendList.empty())
        {
            ...
            for (const FriendInfo& friendInfo : friendList) {
                ...
                if(friendInfo.friendID>9000){
                    ...
                    sendinfo.sql.append(friendInfoString);
                    sendinfo.friendIcon.append(func.getPhoto(friendInfo.friendID));
                }
...
void myServer::getinfo_group(QTcpSocket **tcpSocket, Protocol recvinfo)
{
    Func func;
        QList<FriendInfo>friendList = func.getUserFriendsWithInfo(recvinfo.friendid);
        Protocol sendinfo;
        if(!friendList.empty())
        {
            for (const FriendInfo& friendInfo : friendList) {
                QString friendInfoString ;
                if(friendInfo.isOnline==1)
                {
                    // 将 FriendInfo 结构体转换为字符串格式
                    friendInfoString =  friendInfo.friendUsername+ "\tid" +
                                               QString::number(friendInfo.friendID)+ "[在线]";
                }
                else{
                    friendInfoString =  friendInfo.friendUsername+ "\tid" +
                                               QString::number(friendInfo.friendID)+ "[离线]";
                }
    ...
    if(groupMem.empty()){
        ...
        if(groupId==-1){
            ...
            for (const QString& member : groupMem){
                QList<QString>parts=member.split(" id:");

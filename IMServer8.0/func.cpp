...
// 返回值:
// - 新注册账户的ID,如果有错误则返回-1
int Func::account_register(const QString &username, const QString &password, const QByteArray &image, int question, const QString &answer)
{
    Utls utls;
...
//    QSqlQuery query;
//    query.prepare("INSERT user_information_table VALUES id =:id photo = :photoData WHERE userID = :userID");
//    query.bindValue(":photoData", photoData);
//    query.bindValue(":userID", userID);

    QSqlQuery query;
    query.prepare("INSERT INTO user_information_table (userID, userName, passWord, photo, loginStatus, question, answer) VALUES (:id, :username, :hashedPassword, :photo, 0, :question, :answer)");

    query.bindValue(":id", id);
    query.bindValue(":username", username);
    query.bindValue(":hashedPassword", hashedPassword);
    query.bindValue(":photo", image);

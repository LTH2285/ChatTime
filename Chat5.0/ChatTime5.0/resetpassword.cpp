...
void ResetPassword::connectToServer() {
    ...
    connect(tcpSocket, &QTcpSocket::readyRead, [=]() {
        ...
        if(recvinfo.serverport == 0)
            ui->securityQuestion->setText(QString("他yeye的为什么不显示"));
        else if(recvinfo.serverport == 1)
            ui->securityQuestion->setText(QString("他奶奶的为什么不he"));
        else if(recvinfo.serverport == 2)
            ui->securityQuestion->setText(QString("他yeye的为什么不he"));
        else
            ui->securityQuestion->setText(QString("他奶奶的为什么不显示"));
    });
}
